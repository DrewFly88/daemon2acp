...
1. **AtomCode** 已安装（`D:\AtomCode\atomcode.exe` 或 PATH 中可找到 `atomcode`）
2. **Python** 可用（推荐 `D:\QwenPaw\python.exe`，与 QwenPaw 同环境，已有 aiohttp/pydantic 依赖）
3. **daemon2acp** 项目位于 `D:\代码\daemon2acp\python\`
...
| `args` | 每行一个参数：<br>`D:\代码\daemon2acp\python\stdio_server.py` |
| `env` | 见下表 |
...
| `ATOMCODE_BIN` | `D:\AtomCode\atomcode.exe` | 可选。atomcode 可执行文件绝对路径（PATH 找不到时设） |
...
```
starting atomcode daemon: D:\AtomCode\atomcode.exe daemon --port 13459 ...
```
...
- 脚本路径含中文（`D:\代码\`）无影响，Python 处理 Unicode 路径正常
