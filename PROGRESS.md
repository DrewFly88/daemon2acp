...
- `command` 改为绝对路径（如 `/usr/local/bin/python3` 或本地 Python 路径）
...
| 用途 | 路径 |
|------|------|
| ACP SDK schema（权威） | `acp/schema.py`（pip 安装后可在 `site-packages/acp/` 中找到） |
| ACP SDK method 字符串 | `acp/meta.py` |
| QwenPaw ACP client 实现 | `qwenpaw/agents/acp/` |
| QwenPaw ACP mock runner（范例） | `tests/integration/fixtures/acp_mock_runner.py` |
...
**建议改进**：`command` 用绝对路径（如 `/usr/local/bin/python3`）避免 PATH 依赖。
